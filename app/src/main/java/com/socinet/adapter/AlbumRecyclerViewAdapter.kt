...
class AlbumRecyclerViewAdapter constructor(mListener: OnClickListener, mPosts: MutableList<Album>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private var headerView: View? = null
    private var footerView: View? = null
    var listener: OnClickListener = mListener
    var posts: MutableList<Album> = mPosts
    ...
    fun addPosts(mPosts: List<Album>){
        posts.addAll(mPosts)
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == Type.Header.ordinal) {
            ...
            val ui = AlbumRecyclerViewUI()
            AlbumRecyclerViewHolder(ui, ui.createView(AnkoContext.create(parent.context, parent)))
        }
    }
